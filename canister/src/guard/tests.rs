...
fn should_acquire_and_release_guard() {
    init();
    {
        ...
        assert_in_flight_eq(&[(USER_A, TOKEN_A)]);
    }
    assert_in_flight_eq(&[]);
}
...
fn should_not_block_distinct_token() {
    ...
    assert_in_flight_eq(&[(USER_A, TOKEN_A), (USER_A, TOKEN_B)]);
}
...
fn should_not_block_distinct_caller() {
    ...
    assert_in_flight_eq(&[(USER_A, TOKEN_A), (USER_B, TOKEN_A)]);
}
...
fn assert_in_flight_eq(expected: &[(Principal, TokenId)]) {
    state::with_state(|s| {
