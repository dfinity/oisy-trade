...
mod pair_params {
    use crate::order::{InvalidPairParams, LotSize, TickSize, validate_pair_params};
    use std::num::NonZeroU64;

    fn tick(value: u64) -> TickSize {
        TickSize::new(NonZeroU64::new(value).unwrap())
    }

    fn lot(value: u64) -> LotSize {
        LotSize::new(NonZeroU64::new(value).unwrap())
    }

    /// Trading pairs selected for initial listing must admit exact settlement
    /// under their intended parameters, where
    /// `tick_size = price_increment × 10^quote_decimals` and
    /// `lot_size = quantity_increment × 10^base_decimals`.
    #[test]
    fn selected_trading_pairs_satisfy_settlement_invariant() {
        // (pair, base_decimals, tick_size, lot_size, human tick / lot)
        let pairs = [
            ("ckBTC/ckUSDT", 8u8, 10_000u64, 10_000u64), // $0.01      / 0.0001 BTC
            ("ckETH/ckUSDT", 18, 10_000, 100_000_000_000_000), // $0.01      / 0.0001 ETH
            ("ckUSDC/ckUSDT", 6, 100, 10_000),           // $0.0001    / 0.01 USDC
            ("ICP/ckUSDT", 8, 1_000, 1_000_000),         // $0.001     / 0.01 ICP
            ("VCHF/ckUSDT", 8, 100, 1_000_000),          // 0.0001 USDT/ 0.01 VCHF
            ("ICP/ckBTC", 8, 10, 10_000_000),            // 1e-7 BTC   / 0.1 ICP
            ("ICP/VCHF", 8, 100_000, 1_000_000),         // 0.001 CHF  / 0.01 ICP
        ];
        for (pair, base_decimals, tick_size, lot_size) in pairs {
            assert_eq!(
                validate_pair_params(tick(tick_size), lot(lot_size), base_decimals),
                Ok(()),
                "{pair} should satisfy the exact-settlement invariant"
            );
        }
    }

    #[test]
    fn rejects_indivisible_tick_lot() {
        // Binance BTCUSDT parameters: $0.01 tick (10_000) and 0.00001 BTC lot
        // (1_000) on an 8-decimal base give tick·lot = 10^7, not a multiple of
        // 10^8 (minimum quote increment 0.1 base units), so it would round.
        assert_eq!(
            validate_pair_params(tick(10_000), lot(1_000), 8),
            Err(InvalidPairParams::IndivisibleTickLot {
                tick_size: 10_000,
                lot_size: 1_000,
                base_decimals: 8,
            })
    ...
    #[test]
    fn rejects_base_decimals_that_overflow_the_divisor() {
        // 10^20 exceeds u64, so the settlement divisor is unrepresentable.
        assert_eq!(
            validate_pair_params(tick(1), lot(1), 20),
            Err(InvalidPairParams::BaseDecimalsTooLarge { decimals: 20 })
        );
        // 10^19 still fits u64.
        assert_eq!(
            validate_pair_params(tick(1), lot(10_000_000_000_000_000_000), 19),
            Ok(())
        );
    }

    #[test]
    fn base_decimals_zero_accepts_any_tick_lot() {
        // base_scale = 1 divides everything (the un-scaled, pre-fix semantics).
        assert_eq!(validate_pair_params(tick(7), lot(13), 0), Ok(()));
    }
}
