...
fn should_project_and_append_both_legs_of_a_settlement() {
    ...
    store.append(buy_taker_settlement(0, 1, 0, taker_user(), maker_user()));
...
fn should_index_each_leg_under_its_own_owner() {
    ...
    store.append(buy_taker_settlement(0, 1, 0, alice, bob));
...
fn should_swap_sides_for_a_sell_taker() {
    ...
    store.append(sell_taker_settlement(0, 1, 0, taker_user(), maker_user()));
...
fn should_return_one_orders_trades_newest_first_excluding_other_orders() {
    ...
    store.append(buy_taker_settlement(0, 1, 0, USER, USER));
    store.append(buy_taker_settlement(0, 2, 1, USER, USER));
...
fn should_page_one_orders_trades_via_after_cursor() {
    ...
    let order_a = OrderId::new(BOOK, OrderSeq::new(0));
    for seq in 0..3 {
        store.append(buy_taker_settlement(0, 1, seq, USER, USER));
    }
...
fn should_return_a_users_trades_across_orders_newest_first_scoped_to_owner() {
    ...
    store.append(buy_taker_settlement(0, 2, 0, alice, bob));
    store.append(buy_taker_settlement(1, 2, 1, alice, bob));
...
fn should_page_a_users_trades_via_after_cursor() {
    ...
    for seq in 0..3 {
        store.append(buy_taker_settlement(0, 1, seq, alice, other));
    }
...
fn should_return_an_empty_account_page_for_an_unknown_user() {
    let mut store = store();
    store.append(buy_taker_settlement(
        0,
        1,
        0,
        UserId::new(1),
...
fn should_return_an_empty_account_page_for_a_valid_cursor_with_no_older_trades() {
    let mut store = store();
    let alice = UserId::new(1);
    store.append(buy_taker_settlement(0, 1, 0, alice, UserId::new(2)));
...
fn should_clamp_an_account_page_to_requested_length() {
    ...
    let alice = UserId::new(1);
    for seq in 0..5 {
        store.append(buy_taker_settlement(0, 1, seq, alice, UserId::new(2)));
    }
...
fn settlement(
    ...
    let mut settlement = FillSettlement::new(fill, fee_rates(), base_scale(), BOOK, TIMESTAMP);
    settlement.resolve_owners(taker_user, maker_user);
    settlement
}
