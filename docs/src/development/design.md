...
4. **Canceled**: The user canceled the order via `cancel_limit_order`. `Canceled` is **user-initiated**. Reserved tokens are returned to the user's available balance.
5. **Expired**: A **system-initiated** termination driven by the order's `time_in_force` — the engine could not honor the order's time-in-force constraint. Today the only such case is a fill-or-kill order that cannot fully fill (see Time-in-Force). The order never rests and settles no trade, so no proceeds are exchanged; the placement reservation is released back to the user's available balance. A client distinguishes "I changed my mind" (`Canceled`) from "the engine couldn't honor my time-in-force" (`Expired`).

#### Time-in-Force

Each limit order carries a `time_in_force` policy that constrains **how long the order may rest in the book**. It is evaluated when the matching engine processes the order — *upon execution* — not when `add_limit_order` is called, because this call only enqueues the order; it does not place it in the book.
