...
/// `AddTradingPairEvent` records persisted before DEFI-2726 do not carry
/// the `fee_rates` field; they must replay as if the pair had zero rates.
/// This guards the back-compat contract documented on `AddTradingPairEvent`
/// and on the audit path's `unwrap_or_default()`.
#[test]
fn should_replay_pre_fees_add_trading_pair_event_as_zero_rates() {
    use crate::state::event::AddTradingPairEvent;

    let init = init_event(Mode::GeneralAvailability);
    let legacy_pair = Event {
        timestamp: 2,
        payload: EventType::AddTradingPair(AddTradingPairEvent {
            ...
            // Absent on the wire for events written before DEFI-2726.
            fee_rates: None,
        }),
    };

    let state = replay_events(
        vec![init, legacy_pair],
        order_history(),
        balances(),
        StableMemoryOptions::Write,
    );
    ...
    let book = state.get_order_book(&pair).expect("book must be present");
    assert_eq!(book.fee_rates(), FeeRates::default());
}
