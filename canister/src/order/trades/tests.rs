...
fn should_return_a_users_trades_across_orders_newest_first_scoped_to_owner() {
    ...
    append(&mut store, Side::Buy, 0, 2, 0, alice, bob);
    append(&mut store, Side::Buy, 1, 2, 1, alice, bob);
...
fn should_page_a_users_trades_via_after_cursor() {
    ...
    for seq in 0..3 {
        append(&mut store, Side::Buy, 0, 1, seq, alice, other);
    }
...
fn should_return_an_empty_account_page_for_a_valid_cursor_with_no_older_trades() {
    let mut store = store();
    let alice = UserId::new(1);
    append(&mut store, Side::Buy, 0, 1, 0, alice, UserId::new(2));
...
fn should_clamp_an_account_page_to_requested_length() {
    ...
    let alice = UserId::new(1);
    for seq in 0..5 {
        append(&mut store, Side::Buy, 0, 1, seq, alice, UserId::new(2));
    }
