...
mod fill_or_kill {
    ...
    /// Sell-side FOK end-to-end: a FOK Sell fully fills against a resting Buy
    /// maker, exercising the bids-side matching walk and the Sell settlement.
    #[tokio::test]
    async fn should_fill_fok_sell_against_sufficient_liquidity() {
        let setup = Setup::new().await.with_trading_pair().await;
        // A resting GTC buy provides the bid liquidity the FOK sells into. The
        // buyer reserves `PRICE × quantity / 10^9` quote (base = 9 decimals).
        let buyer = Principal::from_slice(&[0x01]);
        let buyer_client = setup.oisy_trade_client_with_caller(buyer);
        let quote_reserved = 1_000_000_000u64;
        setup
            .deposit_flow(buyer, setup.quote_token_id())
        ...
        // The FOK seller delivers the base; it fully fills against the resting buy.
        let seller = Principal::from_slice(&[0x02]);
        let seller_client = setup.oisy_trade_client_with_caller(seller);
        ...
        let sell = seller_client.get_my_order(sell_id).await.unwrap().order;
        assert_eq!(sell.status, OrderStatus::Filled);
        assert_eq!(sell.filled_quantity, Nat::from(quantity));
        let buy = buyer_client.get_my_order(buy_id).await.unwrap().order;
        assert_eq!(buy.status, OrderStatus::Filled);
        // Seller was paid the whole quote notional; buyer holds the base.
        assert_eq!(
            seller_client
                .get_balance(setup.quote_token_id())
                .await
    ...
    /// Sell-side FOK kill end-to-end: a FOK Sell priced above the only resting
    /// bid cannot cross, is killed to `Expired`, and the BASE reservation is
    /// refunded in full. Exercises `refund_for`'s Sell branch through replay.
    #[tokio::test]
    async fn should_expire_fok_sell_against_insufficient_bid_and_refund_base() {
        let setup = Setup::new().await.with_trading_pair().await;
        // A resting GTC buy at PRICE; the FOK sells at 2 × PRICE and cannot cross.
        let buyer = Principal::from_slice(&[0x01]);
        let buyer_client = setup.oisy_trade_client_with_caller(buyer);
        let quote_reserved = 1_000_000_000u64;
        setup
        ...
        assert_eq!(sell.filled_quantity, Nat::from(0u64));
        // The whole base reservation is released back to the seller's free balance.
        assert_eq!(
            seller_client
                .get_balance(setup.base_token_id())
                .await
        ...
        // The resting bid is untouched: still Open with its quote reserved.
        let buy = buyer_client.get_my_order(buy_id).await.unwrap().order;
        assert_eq!(buy.status, OrderStatus::Open);
        assert_eq!(buy.filled_quantity, Nat::from(0u64));
        assert_eq!(
            buyer_client
                .get_balance(setup.quote_token_id())
                .await
